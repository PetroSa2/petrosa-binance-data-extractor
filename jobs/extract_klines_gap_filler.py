                    ...
                    for chunk_start, chunk_end in weekly_chunks:
                        ...
                        try:
                            chunk_result = self.fill_gap_chunk(
                                symbol, chunk_start, chunk_end, binance_client, db_adapter
                            )
                            ...
                            if chunk_result['success']:
                                ...
                                self.logger.warning(f"Failed to fill chunk for {symbol}: {chunk_result.get('error', 'Unknown error')}")
    ...
    def _run_gap_filling_impl(self) -> Dict:
        ...
        try:
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                future_to_symbol = {
                    executor.submit(self.process_symbol_gaps, symbol, binance_client): symbol
...
Examples:
  ...
  python extract_klines_gap_filler.py --period 15m --weekly-chunk-days 5
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Maximum gap size in days to process (default: 30)"
    )
