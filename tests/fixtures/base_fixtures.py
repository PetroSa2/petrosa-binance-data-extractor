    ...
    for data in realistic_klines_data[:10]:  # Use first 10 for testing
        models.append(
            Kline(
                symbol="BTCUSDT",
                interval="1m",
                open_time=datetime.fromtimestamp(data["openTime"] / 1000, timezone.utc),
                close_time=datetime.fromtimestamp(data["closeTime"] / 1000, timezone.utc),
                ...
                timestamp=datetime.fromtimestamp(data["openTime"] / 1000, timezone.utc),
            )
        )
    return models
    ...
    for data in realistic_funding_data:
        models.append(
            FundingRate(
                ...
                funding_time=datetime.fromtimestamp(data["fundingTime"] / 1000, timezone.utc),
                next_funding_time=datetime.fromtimestamp(
                    data["nextFundingTime"] / 1000
                , timezone.utc),
                timestamp=datetime.fromtimestamp(data["fundingTime"] / 1000, timezone.utc),
            )
        )
    return models
    ...
    for data in realistic_trades_data:
        models.append(
            Trade(
                ...
                trade_time=datetime.fromtimestamp(data["time"] / 1000, timezone.utc),
...
def sample_extraction_metadata() -> ExtractionMetadata:
    ...
    return ExtractionMetadata(
        period="15m",
        start_time=datetime.fromtimestamp(TestingConstants.BASE_TIMESTAMP / 1000, timezone.utc),
        end_time=datetime.fromtimestamp(TestingConstants.BASE_TIMESTAMP / 1000, timezone.utc)
        + timedelta(hours=24),
        total_records=1000,
        gaps_detected=2,
