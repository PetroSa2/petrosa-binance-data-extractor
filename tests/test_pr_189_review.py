    ...
    for manifest_path in manifests:
        ...
        for doc in docs:
            ...
            if 'm15' in doc['metadata']['name']:
                ...
                assert resources['requests']['memory'] == "512Mi", f"Wrong memory request in {manifest_path}"
                assert resources['requests']['cpu'] == "300m", f"Wrong cpu request in {manifest_path}"
                assert resources['limits']['memory'] == "1Gi", f"Wrong memory limit in {manifest_path}"
                assert resources['limits']['cpu'] == "800m", f"Wrong cpu limit in {manifest_path}"
...
def test_k8s_manifest_no_duplicate_env():
    ...
    for filename in os.listdir(k8s_dir):
        ...
        for doc in docs:
            ...
            # Check PodSpec
            if doc['kind'] == 'CronJob':
                check_containers(doc['spec']['jobTemplate']['spec']['template']['spec']['containers'], filename)
            elif doc['kind'] in ['Deployment', 'StatefulSet', 'Job']:
                check_containers(doc['spec']['template']['spec']['containers'], filename)
            elif doc['kind'] == 'Pod':
...
def test_optional_instrumentors_debug_logging():
    ...
    with patch("utils.telemetry.OTEL_AVAILABLE", True):
        ...
        mock_instr = Mock()
        mock_instr.return_value.instrument.side_effect = Exception("Instrumentation failed")
        ...
        with patch("utils.telemetry.RequestsInstrumentor", mock_instr):
            with patch.object(manager.logger, "debug") as mock_debug:
                ...
                debug_calls = [c.args[0] if c.args else "" for c in mock_debug.call_args_list]
                assert any("Skipped optional Requests instrumentor" in str(msg) for msg in debug_calls)
