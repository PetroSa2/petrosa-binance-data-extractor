...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if context_type in ['deploy', 'deployment', 'k8s', 'kubernetes']:
            ...
            print("Run tests: python -m pytest tests/ -v --cov=. --cov-report=term --tb=short")
            print("Fix linting: flake8 . --count --select=E9,F63,F7,F82 --show-source --statistics")
            print("Use GitHub CLI with file output: gh command > /tmp/file.json && cat /tmp/file.json")
